---
title: "Introduction to bootRanges (test build: no genome segmentation, no EnsDb.Hsapiens.v86)"
author: "Wancen Mu"
...
**Note:** This test build omits the "Segmentation by gene density"
section, including its `EnsDb.Hsapiens.v86`, `DNAcopy`, and `RcppHMM`
demonstrations, to test vignette build time with fewer dependencies.
The pre-built `ExperimentHub` segmentation used later is unchanged.
...
In this vignette, we use a pre-built segmentation of the hg38 genome,
perform bootstrap sampling of peak ranges, and evaluate overlaps for
observed peaks and bootstrap peaks. We also provide other examples of
statistics that can be computed with the `bootRanges` framework,
including a single cell multi-omics example and a special case of
bootstrapping features in one region of the genome.
...
which were generated using code outlined in the "Segmentation by gene
density" section of the full `bootRanges` vignette. That section is
omitted here to test build time without `EnsDb.Hsapiens.v86`, `DNAcopy`,
and `RcppHMM`.
