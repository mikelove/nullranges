...
**Note:** This is a scratch/test copy of `vignettes/bootRanges.Rmd`
used to time the vignette build without the "Segmentation by gene
density" section (the CBS/HMM demo built from `EnsDb.Hsapiens.v86`
gene density). It is not part of the package's built vignettes; the
`seg` object used later still comes from the pre-built `ExperimentHub`
segmentation (`EH7307`), so downstream `bootRanges()` calls are
unaffected. To render:
`rmarkdown::render("dev/bootRanges-noseg/bootRanges-noseg.Rmd")` from
the package root (after `R CMD INSTALL .` or `devtools::load_all()`).
...
```{r, echo=FALSE}
knitr::include_graphics("../../vignettes/images/bootRanges.jpeg")
```
...
(see the "Segmentation by gene density" section of the full
`bootRanges` vignette for how these were generated; that section is
omitted here to test build time without `EnsDb.Hsapiens.v86` /
`DNAcopy` / `RcppHMM`).
